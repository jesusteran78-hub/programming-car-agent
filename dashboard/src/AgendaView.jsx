...
function JobCard({ job }) {
    return (
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden flex flex-col transition-all hover:shadow-md hover:-translate-y-1">
            <div className={`h-2 w-full ${job.status === 'confirmed' ? 'bg-emerald-500' : 'bg-amber-400'}`} />
            <div className="p-6 flex-1 flex flex-col">
                <div className="flex justify-between items-start mb-4">
                    <span className="font-mono text-xs font-bold bg-slate-100 px-2 py-1 rounded text-slate-600">{job.time}</span>
                    <span className={`text-xs font-black uppercase tracking-wider ${job.status === 'confirmed' ? 'text-emerald-600' : 'text-amber-500'}`}>
