...
import { Sparkles, Video, Link, Lightbulb, Play, Activity, Clock, Eye, BarChart3, CheckCircle2, AlertTriangle, Terminal } from 'lucide-react'

export default function MarketingView() {
    const [generating, setGenerating] = useState(false)
    const [formData, setFormData] = useState({ title: '', idea: '', image: '' })
    const [logs, setLogs] = useState([
        { id: 1, type: 'video', status: 'ready', title: 'Falla Chevy Silverado - Parte 1', views: 12500, date: 'Hace 2 horas' },
        { id: 2, type: 'video', status: 'ready', title: 'Llave Perdida BMW', views: 8200, date: 'Ayer' },
    ])
                        ...
                        <div className="p-8">
                            <div className="flex items-center gap-3 mb-8">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs text-slate-500 font-bold uppercase tracking-wider">MODO AUTOMÁTICO</p>
                                </div>
                            </div>
                            ...
                            <div className="space-y-6">
                                ...
                                <div className="group/input">
                                    <label className="text-xs font-bold text-slate-500 uppercase mb-2 block group-focus-within/input:text-purple-600 transition-colors">Título del Video</label>
                                    <div className="relative">
                                        ...
                                        />
                                        <Video className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within/input:text-purple-500 transition-colors" size={20} />
                                    </div>
                                </div>

                                {/* URL IMAGEN */}
                                <div className="group/input">
                                    <label className="text-xs font-bold text-slate-500 uppercase mb-2 block group-focus-within/input:text-blue-600 transition-colors">Imagen de Referencia (URL)</label>
                                    <div className="relative">
                                        <input
                                        ...
                                        />
                                        <Link className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within/input:text-blue-500 transition-colors" size={20} />
                                    </div>
                                    <p className="text-[10px] text-slate-400 mt-2 text-right">Dejar vacío para usar imagen genérica de taller.</p>
                                </div>

                                {/* IDEA */}
                                <div className="group/input">
                                    <label className="text-xs font-bold text-slate-500 uppercase mb-2 block group-focus-within/input:text-amber-500 transition-colors">Concepto / Guión</label>
                                    <div className="relative">
                                        <textarea
                                        ...
                                        />
                                        <Lightbulb className="absolute left-4 top-5 text-slate-400 group-focus-within/input:text-amber-500 transition-colors" size={20} />
                                    </div>
                                </div>
                            </div>
                ...
                <div className="lg:col-span-5 flex flex-col gap-6">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-100">
                            <div className="text-slate-400 mb-2"><Eye size={20} /></div>
                        ...
                        <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-100">
                            <div className="text-slate-400 mb-2"><BarChart3 size={20} /></div>
                    ...
                    <div className="bg-white rounded-3xl shadow-sm border border-slate-100 flex flex-col flex-1 min-h-[400px]">
                        <div className="p-6 border-b border-slate-50 flex justify-between items-center">
                            ...
                            <span className="bg-slate-100 text-slate-600 px-2 py-1 rounded-lg text-xs font-bold">Recientes</span>
                        </div>
                        <div className="p-4 space-y-3 overflow-y-auto max-h-[500px]">
                            {logs.map(log => (
                                <div key={log.id} className="group flex gap-4 p-4 rounded-2xl hover:bg-slate-50 transition-colors border border-transparent hover:border-slate-100">
                                    <div className={`w-12 h-12 rounded-xl flex-shrink-0 flex items-center justify-center text-xl shadow-sm ${log.status === 'ready' ? 'bg-green-100 text-green-600' : 'bg-amber-100 text-amber-600'}`}>
                                        {log.status === 'ready' ? <CheckCircle2 size={24} /> : <AlertTriangle size={24} />}
                                    </div>
                                    <div className="flex-1 min-w-0">
                                        <h5 className="font-bold text-slate-900 truncate group-hover:text-purple-700 transition-colors">{log.title}</h5>
                                        <p className="text-xs text-slate-500 mt-1 flex items-center gap-2">
                                            <span>{log.date}</span>
                                            <span className="w-1 h-1 rounded-full bg-slate-300"></span>
                                            <span className="font-bold text-slate-600">{log.views.toLocaleString()} vistas</span>
                                        </p>
                                        {log.script && (
