...
async function getStoredPrice(make, model, year, serviceType = 'copy') {
    try {
        ...
        if (error) {
            console.error('Error fetching stored price:', error.message);
            return null;
        }
        ...
        console.error('Exception in getStoredPrice:', e);
        return null;
    }
}
...
async function learnNewPrice(text) {
    ...
    if (!match) {
        return { success: false, message: "No entendí el formato. Usa: 'Aprende: Marca Modelo Años Tipo Precio' (Ej: Mercedes C300 2015-2018 Copia 250)" };
    }

    const [_, make, model, yearStr, typeRaw, priceStr] = match;
    ...
    if (error) {
        console.error('Error learning price:', error);
