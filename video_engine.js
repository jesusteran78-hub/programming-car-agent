...
async function generateViralVideo(title, idea, imageUrl) {
    console.log(`🏭 FABRICA INICIADA: ${title}`);

    try {
        // PASO 1: GENERAR PROMPT CINEMATOGRÁFICO (OpenAI)
        console.log("🧠 1. Diseñando Escena Cinematic (GPT-4o)...");
        const soraPrompt = await generateSoraPrompt(title, idea);
        console.log(`✅ Prompt de Dirección: "${soraPrompt.substring(0, 50)}..."`);

        // PASO 2: VIDEO (KIE / SORA 2)
        console.log(`🎨 2. Renderizando en KIE API (Sora 2 Async)...`);
        let videoUrl;

        if (process.env.KIE_API_KEY) {
            try {
                // 2.1 CREAR TAREA (ASYNC)
                const createResponse = await axios.post(KIE_CREATE_TASK_URL, {
                    model: "sora-2-image-to-video",
                    input: {
                        prompt: soraPrompt,
                        image_urls: [imageUrl || "https://res.cloudinary.com/dtfbdf4dn/image/upload/v1767748438/ugc-auto/nbdfysted9kuvfcpgy28.png"],
                ...
                console.log(`⏳ Tarea KIE Iniciada. ID: ${taskId}. Esperando renderizado...`);

                // 2.2 POLLING (Esperar a que termine)
                videoUrl = await pollKieTask(taskId);

            } catch (kieError) {
                console.error("❌ ERROR EN KIE API:", kieError.response?.status, kieError.response?.data || kieError.message);
                console.log("⚠️ KIE falló. Usando video de respaldo.");
                videoUrl = "https://res.cloudinary.com/dtfbdf4dn/video/upload/v1767949970/ugc-watermarked/hiu2w9fv9ksvnhrzcvgp.mp4";
            }
        } else {
            console.log("⚠️ MODO SIMULACIÓN (Falta API Key de KIE)");
            videoUrl = "https://res.cloudinary.com/dtfbdf4dn/video/upload/v1767949970/ugc-watermarked/hiu2w9fv9ksvnhrzcvgp.mp4";
        }
        console.log("✅ Video URL Final:", videoUrl);

        // PASO 3: PUBLICACIÓN (BLOTATO)
        console.log("📡 3. Distribuyendo a Redes (Blotato)...");

        // 3.1 Generar Caption Viral
        console.log("✍️ Redactando Copy para Redes...");
        const viralCaption = await generateViralCaption(title, soraPrompt);

        // Parse the JSON (handling potential markdown fences)
        let captions;
        try {
            const cleanJson = viralCaption.replace(/```json/g, '').replace(/```/g, '').trim();
            captions = JSON.parse(cleanJson);
            console.log(`✅ Copy Generado (Main: TikTok): "${captions.tiktok.substring(0, 30)}..."`);
        } catch (e) {
            console.warn("⚠️ Falló el parseo JSON de captions, usando texto crudo como fallback.");
        ...
        if (process.env.BLOTATO_API_KEY) {
            ...
            if (targets.length === 0) {
                console.warn("⚠️ No hay IDs de Blotato configurados para publicar.");
            }

            for (const target of targets) {
                try {
                    console.log(`📡 Publicando en ${target.platform.toUpperCase()}...`);
                    ...
                    const blotatoResponse = await axios.post(BLOTATO_API_URL, {
                        post: {
                            accountId: target.id,
                            content: {
                    ...
                    console.log(`✅ Publicado en ${target.platform}:`, blotatoResponse.data?.id || "OK");
            ...
            console.log("⚠️ MODO SIMULACIÓN (Falta API Key de Blotato)");
        ...
        console.error("❌ ERROR CRÍTICO EN LA FÁBRICA:", error.message);
        throw error;
    }
}
...
async function pollKieTask(taskId) {
    ...
    for (let i = 0; i < maxAttempts; i++) {
        try {
            ...
            console.log(`⏳ Polling KIE (${i + 1}/${maxAttempts}): Status ${taskData?.status}`);

            if (taskData?.status === 3 || taskData?.status === 'SUCCEEDED' || taskData?.status === 'completed' || (taskData?.result && taskData?.result.video_url)) {
                // Success!
                ...
                if (!finalUrl) {
                    // Maybe in a list?
                    if (Array.isArray(taskData.result)) return taskData.result[0];
                }
                return finalUrl;
            }
            ...
            console.warn(`⚠️ Error polling KIE (intento ${i}):`, e.message);
            await new Promise(resolve => setTimeout(resolve, interval));
        }
    }
